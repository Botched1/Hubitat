...
metadata {
	definition (name: "Zooz ZSE41 OPEN CLOSE XS SENSOR", namespace: "Botched1", author: "Jason Bottjen") {
		capability "Actuator"
		capability "Battery"
		capability "Sensor"
		...
		fingerprint  mfr:"027A", prod:"7000", deviceId:"E001", inClusters:"0x5E,0x85,0x8E,0x59,0x55,0x86,0x72,0x5A,0x73,0x80,0x9F,0x71,0x87,0x30,0x70,0x84,0x6C,0x7A"
	}

	preferences {
		input name: "paramLEDIndicatorMode", type: "enum", title: "LED Indicator Mode", multiple: false, options: ["0" : "No LED blink on status change", "1" : "LED blink on status change"], required: false, displayDuringSetup: true
		...
		input name: "paramIndicationDirection", type: "enum", title: "Reverse OPEN/CLOSE Indication", multiple: false, options: ["0" : "Normal", "1" : "Reverse"], required: false, displayDuringSetup: true		
...
def updateConfig() {
	...
	// Set Indication Direction param
	if (paramIndicationDirection==null) {
		paramIndicationDirection = 0
	}	
	cmds.add(zwave.configurationV2.configurationSet(scaledConfigurationValue: paramIndicationDirection.toInteger(), parameterNumber: 5, size: 1).format())
	...
	cmds.add(zwave.configurationV2.configurationSet(scaledConfigurationValue: paramGroup2OnDelay.toInteger(), parameterNumber: 6, size: 4).format())
	...
	cmds.add(zwave.configurationV2.configurationSet(scaledConfigurationValue: paramGroup2OffDelay.toInteger(), parameterNumber: 7, size: 4).format())
	...
	cmds.add(zwave.notificationV8.notificationGet(notificationType: 6, event: 0, v1AlarmType: 0).format())
