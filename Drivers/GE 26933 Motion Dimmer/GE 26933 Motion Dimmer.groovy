...
def zwaveEvent(hubitat.zwave.commands.switchbinaryv1.SwitchBinaryReport cmd) {
    ...
    sendEvent(name: "switch", value: cmd.value ? "on" : "off", isStateChange: true, type: "digital")
}
...
def setLevel(value) {
	...
	if (level && currentSwitch=="off") {
		sendEvent(name: "switch", value: "on", isStateChange: true)
	}
	else if (level==0 && currentSwitch=="on") {
...
def setLevel(value, duration) {
	def valueaux = value as Integer
	def level = Math.min(valueaux, 99).toInteger()
