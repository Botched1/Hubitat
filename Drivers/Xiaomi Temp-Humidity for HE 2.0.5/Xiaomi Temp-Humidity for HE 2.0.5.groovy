 ...
 * and Aqara Temperature Humidity Sensor (Model WSDCGQ11LM)
 * Device Driver for Hubitat Elevation hub
 * Version 0.8.1
 *      - 0.8.1 - JJB: Fixed byte swapping for HE v2.0.5
 *
 *
...
def parse(String description) {
	...
	if (cluster == "0402")
	{
		def valueHex2 = swapHexString(valueHex)	
		map = parseTemperature(valueHex2)
	}
	else if (cluster == "0405")
	{
		def valueHex2 = swapHexString(valueHex)
		map = parseHumidity(valueHex2)
	}
	else if (cluster == "0403")
	{
		def valueHex2 = swapHexString(valueHex)
		map = parsePressure(valueHex2)
	}
...
def swapHexString(hexString) {
    if (hexString == null) return
    def swapped = ""
    for (int i = hexString.length(); i > 0; i -= 2) {
        swapped += hexString.substring(i - 2, i )
     }
     return swapped
}
