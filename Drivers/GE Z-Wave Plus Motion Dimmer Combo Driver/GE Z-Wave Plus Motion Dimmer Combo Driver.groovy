...
def startLevelChange(direction, cd=null) {
    def upDownVal = direction == "down" ? true : false
	
	List<String> cmds = []
	
	if (useChildren) {
		cmds.add(zwave.switchMultilevelV2.switchMultilevelStartLevelChange(ignoreStartLevel: true, startLevel: cd.currentValue("level"), upDown: upDownVal))
	} else {
		cmds.add(zwave.switchMultilevelV2.switchMultilevelStartLevelChange(ignoreStartLevel: true, startLevel: device.currentValue("level"), upDown: upDownVal))
	}
...
def stopLevelChange() {
	...
	cmds.add(zwave.switchMultilevelV2.switchMultilevelStopLevelChange())
	cmds.add(zwave.switchMultilevelV2.switchMultilevelGet().format())
	// Send Commands
	sendToDevice(cmds)
}
